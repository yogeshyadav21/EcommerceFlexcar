...
document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    if (sessionStorage.getItem("cartValue") != null) {
      cartValue = JSON.parse(sessionStorage.getItem("cartValue"));
    }
    if (sessionStorage.getItem("cartPrices") != null) {
      CartPrices = JSON.parse(sessionStorage.getItem("cartPrices"));
    }

    wishlist.forEach((id) => {
      addToWishlist(id, false);
    });
    cart.forEach((id) => {
      addToCart(id, false);
    });
...
function addToCart(id, save = true) {
  ...
  product.style =
    "width: 18rem; margin-left: 50px; margin-bottom: 10px; z-index: 1;";
  product.innerHTML = `
    <div class="card-body">
    <div style=" display: flex; justify-content: space-around; align-items: center; margin-bottom: 10px;">
    <img src="${items[id].imgsrc}" style="height: 80px; width: 80px" alt="..." />
  ...
  if (save) {
    ...
    document
      .getElementById(productId + "-itemCounter")
      .setAttribute("value", CartPrices[productId] / items[id].price);
    document.getElementById(productId + "-quantity").textContent =
      CartPrices[productId] / items[id].price;
    document.getElementById(productId + "-total").textContent =
      "$" + CartPrices[productId];
  }
}
...
function addItem(id) {
  ...
  console.log(
    `cartPrices: ${CartPrices} and productId: ${productId} and cartPrices[productId]: ${CartPrices[productId]}`
  );
